...
def check_round_winner():
    ...
    if all([game_field[0][0] != '-', game_field[0][1] != '-', game_field[0][2] != '-']):
        draw = True
    elif all([game_field[0][0] != '-', game_field[1][1] != '-', game_field[2][2] != '-']):
        draw = True
    elif all([game_field[0][2] != '-', game_field[1][1] != '-', game_field[2][0] != '-']):
        draw = True
    elif all([game_field[2][0] != '-', game_field[2][1] != '-', game_field[2][2] != '-']):
        draw = True
    elif all([game_field[0][0] != '-', game_field[1][0] != '-', game_field[2][0] != '-']):
        draw = True
    elif all([game_field[0][2] != '-', game_field[1][2] != '-', game_field[2][2] != '-']):
        draw = True
...
while True:
    ...
    if check_round_winner():
        game_continue = str(input('Для начала следующего раунда введите Y, для завершения игры, введите N: ')).upper()
